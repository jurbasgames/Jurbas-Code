...
class JurbasTUI(App):
    ...
    def run_agent(self, user_input: str) -> None:
        ...
        def on_tool_call(name, args, error=None):
            nonlocal current_ai_msg, current_ai_text, accumulated_stream
        ...
        def stream_handler(text: str):
            nonlocal current_ai_msg, current_ai_text, accumulated_stream
            accumulated_stream += text
            # Strip "AI: " prefixes that agent might emit
            clean_text = accumulated_stream.replace("AI: ", "")

...
class JurbasTUI(App):
    ...
    def run_agent(self, user_input: str) -> None:
        ...
        def confirm_handler(name, args):
            event = threading.Event()
            result = [False]

            def show_modal():
                def on_dismiss(approved):
                    result[0] = approved
                    event.set()

                self.push_screen(ConfirmationModal(name, args), on_dismiss)
...
class ConfirmationModal(ModalScreen[bool]):
    """A modal dialog to confirm a mutating action."""
    ...
    def compose(self) -> ComposeResult:
        message = f"⚠️ The agent wants to perform a mutating action:\n\n"
        if self.tool_name == "run_bash":
            message += f"**$ {self.tool_args.get('command', '')}**"
        elif self.tool_name == "write_file":
            content = self.tool_args.get("content", "")
            message += f"**write_file**: {self.tool_args.get('file_path', '')} ({len(content)} chars)"
        else:
            message += f"**{self.tool_name}**: {self.tool_args}"

        with Grid(id="modal-dialog"):
            yield Static(message, id="modal-message")

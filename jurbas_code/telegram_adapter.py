...
class TelegramAdapter:
    def __init__(self, token: str, agent_factory, allowed_users: list[int] = None):
        self.token = token
    ...
    def _is_authorized(self, update: dict) -> bool:
        """Checks if the user or chat is authorized to use the bot."""
        ...
        # Also check chat_id if user_id not found or not in allowed list
        chat_id = None
        if message:
            chat_id = message.get("chat", {}).get("id")
        elif callback_query:
    ...
    def handle_update(self, update: dict) -> None:
        ...
        if not self._is_authorized(update):
            ...
            if chat_id:
                print(f"Unauthorized access attempt from user_id={user_id}, chat_id={chat_id}")
                self.send_message(chat_id, "❌ **Access Denied**. You are not authorized to use this bot.")

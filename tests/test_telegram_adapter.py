...
def test_telegram_adapter_acl_authorized_message():
    mock_agent = MagicMock()
    adapter = TelegramAdapter("token", lambda: mock_agent, allowed_users=[123])
    ...
    with patch.object(adapter, "_api_call") as mock_api_call:
        ...
        adapter.handle_update(update)
        # Should be processed, so it should attempt to call agent or send response
        mock_agent.chat.assert_called_once()


def test_telegram_adapter_acl_unauthorized_message():
    mock_agent = MagicMock()
    adapter = TelegramAdapter("token", lambda: mock_agent, allowed_users=[123])
    ...
    with patch.object(adapter, "_api_call") as mock_api_call:
        ...
        adapter.handle_update(update)
        # Should NOT be processed by agent
        mock_agent.chat.assert_not_called()
        # Should send Access Denied
...
def test_telegram_adapter_acl_authorized_callback():
    mock_agent = MagicMock()
    adapter = TelegramAdapter("token", lambda: mock_agent, allowed_users=[123])
    adapter.pending_approvals[123] = {"name": "tool", "args": {}, "agent": mock_agent}
    ...
    with patch.object(adapter, "_api_call") as mock_api_call:
        ...
        adapter.handle_update(update)
        # Should be processed (editMessageText is part of successful callback handling)
        mock_api_call.assert_any_call("editMessageText", ANY)


def test_telegram_adapter_acl_unauthorized_callback():
    mock_agent = MagicMock()
    adapter = TelegramAdapter("token", lambda: mock_agent, allowed_users=[123])
    ...
    with patch.object(adapter, "_api_call") as mock_api_call:
        ...
        adapter.handle_update(update)
        # Should send Access Denied and answer callback query with alert
        mock_api_call.assert_any_call("sendMessage", {
            "chat_id": 999,
            "text": "❌ **Access Denied**. You are not authorized to use this bot."
        })
        mock_api_call.assert_any_call("answerCallbackQuery", {
...
def test_telegram_adapter_acl_open_mode():
    mock_agent = MagicMock()
    # No allowed_users means open
    adapter = TelegramAdapter("token", lambda: mock_agent, allowed_users=[])

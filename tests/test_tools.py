from jurbas_code import security
import pytest
...
def test_safe_path_allowed():
    ...
    with patch("os.path.realpath", side_effect=lambda x: os.path.abspath(x)):
        with patch(
            "os.path.commonpath", return_value=os.path.abspath(tools.ALLOWED_BASE)
        ):
...
def test_write_file_success(
    mock_open, mock_getsize, mock_copy2, mock_exists, mock_makedirs, mock_safe_path
):
    mock_safe_path.return_value = "/allowed/test.txt"
...
def test_is_readonly_bash():
    assert security._is_readonly_bash("ls -la") is True
    assert security._is_readonly_bash("rm -rf /") is False
    assert security._is_readonly_bash("ls -la && echo ok") is False


def test_requires_confirmation():
    assert (
        security._requires_confirmation(
            "write_file", {"file_path": "a.txt", "content": "a"}
        )
        is True
    )
    assert security._requires_confirmation("run_bash", {"command": "ls"}) is False
    assert security._requires_confirmation("run_bash", {"command": "rm -rf /"}) is True


@patch("builtins.input", return_value="y")
def test_confirm_action(mock_input):
    assert security.confirm_action("run_bash", {"command": "ls"}) is True
